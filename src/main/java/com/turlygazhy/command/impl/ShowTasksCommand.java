...
//                sendMessage(81, chatId, bot); // Ваши задания
    ...
    private void rejectTask(Bot bot) throws SQLException, TelegramApiException {
        sendMessage("Причина", chatId, bot);
    ...
    private void sendTask(Task task, Bot bot) throws SQLException, TelegramApiException {
        ...
        if (taskType == 0) {
            ...
            bot.sendMessage(new SendMessage()
                    ...
                    .setParseMode(ParseMode.HTML)
                    .setChatId(chatId)
                    .setReplyMarkup(keyboardMarkUpDao.select(messageDao.getMessage(119).getKeyboardMarkUpId())));
        }
    }
    ...
    private ReplyKeyboard getInlineKeyboard(List<Task> tasks) throws SQLException {
        ...
        for (Task task: tasks) {
            ...
            button.setCallbackData("/id" + task.getId());
            row.add(button);
            rows.add(row);
        }
        keyboard.setKeyboard(rows);
